...
class ResNetC4Model(DetectionModel):
    ...
    def build_graph(self, *inputs):
        ...
        featuremap = resnet_c4_backbone(image, cfg.BACKBONE.RESNET_NUM_BLOCK[:3], cfg.BACKBONE.RESNET_BOTTLENECK)
        ...
        feature_fastrcnn = resnet_conv5(roi_resized, cfg.BACKBONE.RESNET_NUM_BLOCK[-1], cfg.BACKBONE.RESNET_BOTTLENECK)    # nxcx7x7
        ...
        if is_training:
            ...
            if cfg.MODE_MASK:
                ...
                feature_maskrcnn = resnet_conv5(roi_resized, cfg.BACKBONE.RESNET_NUM_BLOCK[-1], cfg.BACKBONE.RESNET_BOTTLENECK)
                mask_logits = maskrcnn_upXconv_head(
...
class SeResNetFPNModel(DetectionModel):
    ...
    def build_graph(self, *inputs):
        ...
        c2345 = resnet_fpn_backbone(image, cfg.BACKBONE.RESNET_NUM_BLOCK, cfg.BACKBONE.RESNET_BOTTLENECK)
...
import time
def predictList(pred_func, input_dir, sav_dir, unclass_dir):
    fileList = os.listdir(input_dir)

    stt = time.time()

    for fnm in fileList:
        input_file = os.path.join(input_dir, fnm)
        img = cv2.imread(input_file, cv2.IMREAD_COLOR)
        results = detect_one_image(img, pred_func)
        final = draw_final_outputs(img, results, threshold=0.5)
        # viz = np.concatenate((img, final), axis=1)
        # tpviz.interactive_imshow(viz)
        if final is None:
            cv2.imwrite(os.path.join(unclass_dir, fnm), img)
        else:
            cv2.imwrite(os.path.join(sav_dir, fnm), final)

    duration = time.time()-stt
    print('total time', duration, 'avg time', '%.2f'%(duration/len(fileList)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', help='log directory', default='train_log/maskrcnn_'+today)
    ...
    MODEL = SeResNetFPNModel() if cfg.MODE_FPN else ResNetC4Model()
    ...
    if args.visualize or args.evaluate or args.predict:
        ...
        if args.visualize:
            ...
            if args.evaluate:
                ...
                img_sav_dir = '/woo/dev2/xray/incheon/DemoData/test_result_mask/cls'
                unclass_dir = '/woo/dev2/xray/incheon/DemoData/test_result_mask/un'
                predictList(pred, input_dir=args.predict, sav_dir=img_sav_dir, unclass_dir=unclass_dir)
        ...
        # epochs = 200
        epochs = cfg.TRAIN.LR_SCHEDULE[-1] * factor // stepnum
